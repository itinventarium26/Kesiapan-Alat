...
// Helper function untuk mendapatkan format "Minggu ke-X tahun YYYY" standar ISO (cth: 2026-W38)
...
export default function AdminDashboard() {
  ...
  // --- STATE FILTER LAPORAN DINAMIS ---
  const [filterType, setFilterType] = useState<'hari' | 'minggu' | 'bulan'>('bulan');
  
  // Menyiapkan nilai default hari ini berdasarkan filter
  const todayStr = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  const currentWeek = getISOWeek(new Date()); // YYYY-Www
  ...
  // Fungsi merubah tipe filter dan mereset value input agar tidak error
  ...
  // 1. FILTERING RIWAYAT (Dijalankan otomatis)
  const displayedLogs = logsList.filter(log => {
    if (!filterValue) return true; // Jika dikosongkan, tampilkan semua
    ...
    if (filterType === 'hari') {
      ...
      return getISOWeek(new Date(log.created_at)) === filterValue;
    }
    return true;
  });

  // 2. DOWNLOAD LAPORAN BERDASARKAN FILTER YANG TAMPIL
  ...
  const handleTambahAlat = async (e: React.FormEvent) => {
    ...
    else { showToast("Alat berhasil ditambahkan!", "success"); setKodeAlat(''); setNamaAlat(''); setStatusAlat('siap'); fetchData(); }
  ...
  const handleHapusAlat = async (id: string, kode: string) => {
    ...
    else { showToast(`Alat ${kode} dihapus.`, "success"); fetchData(); }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 w-full relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 text-center md:text-left">
        ...
        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
          {/* Tombol Download CSV Aktif - Mengambil dari filter */}
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/5 text-sm md:text-base">
                ...
                teknisiList.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-100/50 dark:hover:bg-white/5">
          ...
          <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4 bg-white/40 dark:bg-black/20 p-4 rounded-xl border border-slate-200 dark:border-white/5">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              ...
              <select 
                ...
                className="w-full sm:w-auto px-3 py-2 rounded-lg bg-white/70 dark:bg-black/60 border border-slate-300 dark:border-white/20 outline-none dark:text-white text-sm font-medium cursor-pointer focus:ring-2 focus:ring-blue-500"
              >
              ...
              {/* Input Value Filter yang menyesuaikan tipe yang dipilih */}
              <input 
                type={filterType === 'hari' ? 'date' : filterType === 'minggu' ? 'week' : 'month'} 
                value={filterValue}
                onChange={(e) => setFilterValue(e.target.value)}
                className="w-full sm:w-auto px-3 py-2 rounded-lg bg-white/70 dark:bg-black/60 border border-slate-300 dark:border-white/20 outline-none dark:text-white text-sm font-medium focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/5 text-sm md:text-base">
                ...
                displayedLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-100/50 dark:hover:bg-white/5">
                    ...
                    <td className="p-3 md:p-4">
                      <div className="font-bold text-slate-900 dark:text-white">{log.equipments?.kode_alat || 'Alat Dihapus'}</div>
                      <div className="text-xs text-slate-500">{log.equipments?.nama_alat}</div>
                    </td>
                    <td className="p-3 md:p-4">
                      <span className="uppercase text-xs font-bold px-2 py-1 rounded bg-slate-200 text-slate-700 dark:bg-white/10 dark:text-slate-300">
                        {log.status_baru === 'maintenance' ? 'MAINTENANCE' : log.status_baru === 'tidak_siap' ? 'BREAK DOWN' : 'SIAP OPERASI'}
                      </span>
                    </td>
