...
export default function Login() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (isResetMode) {
      ...
      // MENGUBAH ALAMAT REDIRECT KE HALAMAN UPDATE PASSWORD BARU
      ...
      const { data: authData, error: authError } = await supabase.auth.signInWithPassword({ email: finalLoginEmail, password });
    ...
    <div className="relative flex flex-col items-center justify-center px-4 w-full min-h-screen">
      <div className="w-full max-w-md bg-white/60 dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-8 z-10 mb-16">
          ...
          {!isResetMode && (
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-3 rounded-lg bg-white/50 dark:bg-black/20 border border-slate-300 dark:border-white/10 outline-none focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white" placeholder="••••••••"
              />
            </div>
          )}
        ...
        <div className="mt-6 text-center space-y-2">
          {isResetMode && (<button onClick={() => setIsResetMode(false)} className="text-sm text-slate-600 dark:text-slate-400 hover:underline block w-full">Kembali ke Login</button>)}
          <Link href="/" className="text-sm text-blue-600 dark:text-blue-400 hover:underline block">&larr; Kembali ke Dashboard Publik</Link>
        </div>
      </div>
